...
if (!$id) { mensagem('erro', 'Notícia não encontrada.'); redirecionar('index.php'); }

$stmt = $pdo->prepare("SELECT * FROM noticias WHERE id = ?");
$stmt->execute([$id]);
$noticia = $stmt->fetch();
if (!$noticia) { mensagem('erro', 'Notícia não encontrada.'); redirecionar('index.php'); }
if ($_SESSION['usuario_id'] != $noticia['autor'] && !ehAdmin()) { mensagem('erro', 'Permissão negada.'); redirecionar('index.php'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        if ($stmt->execute([$titulo, $conteudo, $imagem, $id])) {
            mensagem('sucesso', 'Notícia atualizada! ✏️'); redirecionar('noticia.php?id='.$id);
...
<head>
    ...
    <title>Editar Notícia - Impacto Verde</title>
...
<body>
    ...
    <div class="auth-container">
        <div class="auth-card">
            <div class="auth-header"><i class="bi bi-pencil-square"></i><h3>Editar Notícia</h3><p>Atualize sua publicação</p></div>
            <?php exibirFlash(); ?>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="noticia.php?id=<?= $noticia['id'] ?>" class="btn btn-outline btn-lg"><i class="bi bi-x-circle"></i> Cancelar</a>
                </div>
            </form>
        </div>
    </div>
