...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileType, $allowedTypes)) {
            ...
            $novoNome = 'perfil_' . $usuario_id . '_' . time() . '.' . $extensao;
            $pasta = 'imagens/';
    ...
    if (empty($erros)) {
        if ($nova_senha !== '') {
            ...
            $stmt->execute([$nome, $email, $bio, $foto, $hash, $usuario_id]);
        } else {
            $stmt = $pdo->prepare("UPDATE usuarios SET nome=?, email=?, bio=?, foto=? WHERE id=?");
            $stmt->execute([$nome, $email, $bio, $foto, $usuario_id]);
        }
        
        $_SESSION['usuario_nome'] = $nome;
        mensagem('sucesso', 'Perfil atualizado com sucesso! ✓');
...
<head>
    ...
    <title>Meu Perfil - Impacto Verde</title>
            ...
            <nav>
                <ul>
                    ...
                    <?php if(estaLogado()): ?>
                        ...
                        <?php endif; ?>
                        <li><a href="perfil.php" class="ativo"><i class="bi bi-person"></i> Perfil</a></li>
        ...
        <div class="perfil-card">
            <div class="perfil-header">
                <div class="perfil-avatar">
                    <?php if(!empty($usuario['foto']) && file_exists($usuario['foto'])): ?>
                        <img src="<?= htmlspecialchars($usuario['foto']) ?>?t=<?= time() ?>" alt="<?= htmlspecialchars($usuario['nome']) ?>">
                    <?php else: ?>
                ...
                <div class="perfil-info">
                    ...
                    <p>📅 Membro desde <?= formatarData($usuario['criado_em']) ?></p>
                    <?php if(!empty($usuario['bio'])): ?>
                        <p class="mt-2"><i class="bi bi-info-circle"></i> <?= nl2br(htmlspecialchars($usuario['bio'])) ?></p>
                    <?php endif; ?>
                </div>
            ...
            <form method="POST" enctype="multipart/form-data" style="margin-top: 1.5rem;">
                ...
                <div class="form-group">
                    ...
                    <?php if(!empty($usuario['foto']) && file_exists($usuario['foto'])): ?>
                        <div class="mb-2">
                            <img src="<?= htmlspecialchars($usuario['foto']) ?>?t=<?= time() ?>" alt="Atual" style="max-height: 100px; border-radius: 8px; border: 2px solid var(--verde-medio);">
