...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $erros = [];
    if (strlen($titulo) < 5) $erros[] = 'Título deve ter pelo menos 5 caracteres.';
    if (strlen($conteudo) < 20) $erros[] = 'Conteúdo deve ter pelo menos 20 caracteres.';
    ...
    if (isset($_FILES['imagem']) && $_FILES['imagem']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileType, $allowed)) $erros[] = 'Formato inválido. Use JPG, PNG ou WebP.';
        elseif ($_FILES['imagem']['size'] > $maxSize) $erros[] = 'A imagem deve ter no máximo 2MB.';
        else {
            $ext = pathinfo($_FILES['imagem']['name'], PATHINFO_EXTENSION);
            $novo = 'noticia_' . $_SESSION['usuario_id'] . '_' . time() . '.' . $ext;
            $pasta = 'imagens/';
    ...
    if (empty($erros)) {
        ...
        if ($stmt->execute([$titulo, $conteudo, $_SESSION['usuario_id'], $imagem])) {
            mensagem('sucesso', 'Notícia publicada com sucesso! 🎉');
            redirecionar('index.php');
        } else mensagem('erro', 'Erro ao publicar.');
...
<head>
    ...
    <title>Nova Notícia - Impacto Verde</title>
        ...
        <div class="container">
            ...
            <nav>
                <ul>
                    ...
                    <?php if(estaLogado()): ?>
                        <?php if(ehAdmin()): ?><li><a href="admin.php" class="btn-destaque"><i class="bi bi-shield-lock"></i> Admin</a></li><?php endif; ?>
                        ...
                        <li><a href="nova_noticia.php" class="btn-destaque ativo"><i class="bi bi-plus-circle"></i> Publicar</a></li>
    ...
    <div class="auth-container">
        <div class="auth-card">
            <div class="auth-header">
                ...
                <p>Compartilhe uma notícia com o mundo</p>
            </div>
            <?php exibirFlash(); ?>
            <form method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label><i class="bi bi-card-heading"></i> Título *</label>
                    <input type="text" name="titulo" required placeholder="Título atrativo" value="<?= isset($_POST['titulo']) ? htmlspecialchars($_POST['titulo']) : '' ?>">
                </div>
                <div class="form-group">
                    <label><i class="bi bi-image"></i> Imagem (opcional)</label>
                    <input type="file" name="imagem" accept="image/*" class="form-control">
                    <small>Formatos: JPG, PNG, WebP. Máx: 2MB</small>
                </div>
                <div class="form-group">
                    <label><i class="bi bi-file-text"></i> Conteúdo *</label>
                    <textarea name="noticia" required placeholder="Escreva sua notícia aqui..."><?= isset($_POST['noticia']) ? htmlspecialchars($_POST['noticia']) : '' ?></textarea>
                </div>
                <div class="d-flex gap-2 flex-wrap">
                    <button type="submit" class="btn btn-lg"><i class="bi bi-check-circle"></i> Publicar</button>
                    <a href="index.php" class="btn btn-outline btn-lg"><i class="bi bi-x-circle"></i> Cancelar</a>
                </div>
            </form>
        </div>
    </div>
